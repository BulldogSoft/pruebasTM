	...
	def cancel_reserved_stock(self):
		...
		for picking in picking_ids:
			...
			if stock_return:
				...
				if new_picking.mapped('move_lines').filtered(lambda move: move.state in ['confirmed', 'waiting','assigned']):
					for stock_lines in new_picking.move_lines:
						stock_lines.write({'quantity_done':stock_lines.product_uom_qty})
