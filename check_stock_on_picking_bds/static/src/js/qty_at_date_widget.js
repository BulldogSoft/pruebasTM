odoo.define('check_stock_on_picking_bds.QtyAtDateWidget', function (require) {
"use strict";
...
var QtyAtDateWidget = Widget.extend({
    template: 'check_stock_on_picking_bds.qtyAtDate',
    ...
    _setPopOver: function () {
        ...
        var $content = $(QWeb.render('check_stock_on_picking_bds.QtyDetailPopOver', {
            data: this.data,
        }));
            ...
            args: [self.data, self.data.product_id.id, self.data.product_stock_quant_ids.res_ids],
                ...
                if (response.length>0)
                    response.map(function(location){
                        ...
                        if(location['qty'] > 0){
                            $content.find('#quantities_table').append("<tr><td>"+location['name']+"</td><td>"+location['qty']+"</td></tr>");
                        }
                    });
                else
                        $content.find('#quantities_table').append("No se encontraron bodegas!");
            });
